...
<OrderProductItemRecyclerAdapter.RecyclerViewHolder>() {
  ...
  override fun onBindViewHolder(holder: RecyclerViewHolder, position: Int) {
    ...
    cartItemRecyclerAdapter =
      OrderItemRecyclerAdapter(context, productItems[position].cart_products, showOriginalPrice)
    ...
    if(showOriginalPrice){
      for (it in productItems[position].cart_products) {
        chargeTotal += ((it.product_item_price - it.product_arpan_profit) * it.product_item_amount)
      }
    }else{
      for (it in productItems[position].cart_products) {
